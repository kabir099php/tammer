...
class ItemController extends Controller
{
    ...
    public function get_products_demo ( Request $request)
    {
        
        $limit = $request->limit ? $request->limit : 30;
        $store_id = $request->store_id ? $request->store_id : 5 ; 
        
        $products  = Item::where('store_id',$store_id)->limit($limit)->get();
    ...
    {
        $limit = $request->limit ? $request->limit : 30;
        $products  = Item::where('store_id',$request->store_id)->limit($limit)->get();
         // Create a new blank PHP object (stdClass)
        $responseObject = new \stdClass();

        // Add the $products collection as a property to the new object
        $responseObject->data = $products; // You can name this property anything, e.g., 'products', 'items', 'data'
        $responseObject->status = true; // You can name this property anything, e.g., 'products', 'items', 'data'
        $responseObject->message = "product listing"; // You can name this property anything, e.g., 'products', 'items', 'data'

        // Return the new object as a JSON response
        // When Laravel's response()->json() receives an object, it serializes it directly.
