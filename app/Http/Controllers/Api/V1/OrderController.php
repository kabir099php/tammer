...
class OrderController extends Controller
{
    ...
    public function place_order(Request $request)
    {
        ...
        {
            $guest =  new Guest();
            $guest->contact_name = $request->contact_person_name;
            $guest->contact_email = $request->contact_person_email;
            $guest->contact_number = $request->contact_person_number;
            $guest->save();
...
//            'contact_person_number' => $request->contact_person_number ? ($request->user ? $request->contact_person_number :str_replace('+', '', $request->contact_person_number)) : ($request->user?$request->user->phone:''),
        ...
        $order->user_id = $request->user ? $request->user->id : $guest->id;
                        ...
                        if ($product->module->module_type == 'food' && $product->food_variations) {
                            ...
                            if (config('module.' . $product->module->module_type)['stock']) {
                                ...
                                $product_data[] = [
                                    'item' => clone $product,
                                    'quantity' => $c['quantity'],
                                    'variant' => null // count($c['variation']) > 0 ? $c['variation'][0]['type'] : null
                                ];
                            }

                            $product->tax =  0 ;//$store->tax;
                            $product = Helpers::product_data_formatting($product, false, false, app()->getLocale());
                           ...
                           // $addon_data = Helpers::calculate_addon_price(\App\Models\AddOn::whereIn('id', $c['add_on_ids'])->get(), $c['add_on_qtys']);
                            ...
                            $total_addon_price +=  0 ;//$or_d['total_add_on_price'];
                            $product_price += $price * $or_d['quantity'];
                            $store_discount_amount += $or_d['discount_type']!='flash_sale'?$or_d['discount_on_item'] * $or_d['quantity']:0;
                            $flash_sale_admin_discount_amount += $or_d['discount_type']=='flash_sale'?$product_discount['admin_discount_amount'] * $or_d['quantity']:0;
                            $flash_sale_vendor_discount_amount += $or_d['discount_type']=='flash_sale'?$product_discount['vendor_discount_amount'] * $or_d['quantity']:0;
...
//        })
    ...
    {
        $guest = Guest::where('contact_email',$request->customer_email)->where( 'contact_name' , $request->customer_name)->first();
        
        if(is_null($guest))
        {
            $guest =  new Guest();
            $guest->contact_name = $request->customer_name;
            $guest->contact_email = $request->customer_email;
            $guest->contact_number = $request->customer_phone;
            $guest->save();
        }

        if($request->items &&  count($request->items )  > 0 )
        {
            foreach ($request->items as $key => $products) {
                
                $cart = new Cart();
                $cart->user_id = $guest->id ; 
                $cart->module_id = 1 ; 
                $cart->is_guest = 1 ; 
                $cart->item_id = $products['id'] ; 
                $cart->price = $products['price_per_kg'] ; 
                $cart->quantity = $products['quantity'] ; 
                $cart->save();
                
                $item = Item::find($products['id']);
        ...
        $order_request['guest_id'] = $guest->id;
        ...
        if($order->getStatusCode() == 200)
        {
            ...
            if(isset($request->type) && $request->type =="POS" )
            {
            ...
            else
            {
                session()->put('last_placed_order_id', $order->original['order_id']);
               session::put('order_id', $order->original['order_id']);
                     //dd(session()->all(), 'Session state in OrderController after storing');
               $cart['order_id'] = $order->original['order_id']; 
        ...
        else
        {
            ...
            return response()->json($order->original, 500);
            }
            else
            {
                return response()->json($order->original, 500);
