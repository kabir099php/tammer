...
class VendorController extends Controller
{
    ...
    public function showDashboard($vendor)
    {
        // Lookup vendor in DB
        dd($vendor);
        $vendorData = Vendor::where('name', $vendor)->firstOrFail();

        return view('vendor.dashboard', compact('vendorData'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $store->vendor_name_ar =  isset($request->name[2]) ? $request->name[2] : null  ;
        ...
        $store->zone_id = 1;
        $store->module_id = 1;
        $store->pickup_zone_id = null ;
        $store->tax = 0;
        $store->delivery_time = 0;
        $store->status = 1;
