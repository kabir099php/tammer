...
|
*/
// Route::domain('{vendor}.trymajlis.com')->group(function () {
//     Route::get('/', [HomeController::class, 'subdomain']);
...
Route::get('/barcode-test', function (Request $request) {
    $currentLang = 'ar'; // Or retrieve from session/user preferences
    // $itemId = $request->query('item_id'); // Get item_id from URL query parameter
    // $orderId = $request->query('order_id'); // Get order_id from URL query parameter
    //return view('feedback', compact('currentLang', 'itemId', 'orderId'));
    return view('new_code.barcode-new', compact('currentLang'));
})->name('barcode-test');
...
Route::get('scanner-checkout', 'HomeController@showCheckout')->name('scan-checkout');
Route::get('thank-you', 'HomeController@thankyou')->name('thankyou');
Route::get('fail', 'HomeController@fail')->name('fail');
Route::get('payment-processing/{order_id}', 'HomeController@paymentProcessing')->name('paymentProcessing');
Route::get('/payment/check-status/{order_id}', 'HomeController@checkOrderStatus')->name('order.status_check');
// In routes/web.php
Route::get('/feedback', function (Request $request) {
    $currentLang = 'en'; // Or retrieve from session/user preferences
    // $itemId = $request->query('item_id'); // Get item_id from URL query parameter
    // $orderId = $request->query('order_id'); // Get order_id from URL query parameter
    //return view('feedback', compact('currentLang', 'itemId', 'orderId'));
    return view('new_code.feedback', compact('currentLang'));
})->name('feedback-page');
...
Route::domain('{branch}.waslqr.com')->group(function () {
    Route::get('checkout-vendor', 'HomeController@checkoutVendor')->name('checkout');
    
});
Route::get('checkout', 'HomeController@checkout')->name('checkout');
Route::post('checkout/process', 'HomeController@processCheckout')->name('processCheckout');    
