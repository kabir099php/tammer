...
<head>
    ...
    <style>
        ...
        .star { color: #FFD700; font-size: 1.5rem; margin: 0 0.1rem; }
...
<body class="p-4 sm:p-6 md:p-8 bg-gradient-to-br from-indigo-50 to-purple-50">
    ...
    <div class="main-container w-full bg-white rounded-xl shadow-2xl overflow-hidden md:flex transform transition-all duration-300 hover:scale-[1.005]">
        <div class="absolute top-4 right-4 z-10 flex space-x-2 fade-in delay-0" style="display:none">
            <button id="lang-en" class="px-4 py-2 bg-purple-500 text-white rounded-lg shadow-md hover:bg-purple-600 focus:outline-none focus:ring-2 focus:ring-purple-400 focus:ring-opacity-75 transition duration-200">EN</button>
            <button id="lang-ar" class="px-4 py-2 bg-gray-200 text-gray-700 rounded-lg shadow-md hover:bg-gray-300 focus:outline-none focus:ring-2 focus:ring-gray-400 focus:ring-opacity-75 transition duration-200">AR</button>
        </div>
        ...
        <div class="p-6 sm:p-8 flex flex-col md:w-1/2 lg:w-2/5 justify-between bg-gradient-to-br from-purple-600 to-indigo-700 text-white relative rounded-t-xl md:rounded-l-xl md:rounded-tr-none fade-in delay-100">
            <div>
                <div class="flex items-center space-x-4 mb-6">
                    ...
                    <div>
                        ...
                        <p style="margin-right:10px" class="text-indigo-200 text-lg" data-key="sellerTagline"></p>
                    </div>
                </div>

                <div class="mb-8">
                    <p class="text-lg text-white mb-2 flex items-center" id="contact-row">
                        <svg class="w-5 h-5 text-indigo-200 rtl-icon-contact" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path d="M2 3a1 1 0 011-1h2.153a1 1 0 01.986.836l.74 4.435a1 1 0 01-.54 1.06l-1.548.774a11.037 11.037 0 006.105 6.105l.774-1.548a1 1 0 011.059-.54l4.435.74a1 1 0 01.836.986V17a1 1 0 01-1 1h-2C7.82 18 2 12.18 2 5V3z"></path></svg>
                        <span data-key="contactLabel"></span> <span class="font-semibold" data-key="contactNumber"></span>
                    </p>
                    <p class="text-lg text-white mb-2 flex items-center" id="location-row">
                        <svg class="w-5 h-5 text-indigo-200 rtl-icon-location" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M5.05 4.05a7 7 0 119.9 9.9L10 18.9l-4.95-4.95a7 7 0 010-9.9zM10 11a2 2 0 100-4 2 2 0 000 4z" clip-rule="evenodd"></path></svg>
                        <span data-key="locationLabel"></span> <span class="font-semibold" data-key="locationValue"></span>
                    </p>

                    <p class="text-lg text-white mb-2 flex items-center" id="cr-row">
                        <svg class="w-5 h-5 text-indigo-200 rtl-icon-location" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                            <path d="M10 2a8 8 0 00-8 8c0 2.45 1.135 4.743 3 6.326V20l5-2.5 5 2.5v-1.674c1.865-1.583 3-3.876 3-6.326a8 8 0 00-8-8zM7 9a1 1 0 100-2 1 1 0 000 2zm6 0a1 1 0 100-2 1 1 0 000 2zm-3 6a2 2 0 100-4 2 2 0 000 4z"/>
                        </svg>
            ...
            <div class="text-center mt-auto fade-in delay-300">
                <h2 class="text-2xl font-bold text-white mb-4 border-b-2 border-indigo-300 pb-2 heading-rtl-align" data-key="qrCodeHeading"></h2>
                <div class="bg-white p-6 rounded-lg inline-block shadow-lg border-2 border-purple-300" style="padding: 1rem !important">
                    <div id="qrcode" class="w-44 h-44 rounded-md mx-auto transform transition-transform duration-300 hover:scale-105" style="height:10rem !important;"></div>
                    <p class="mt-4 text-gray-600 text-sm" data-key="qrCodeDescription"></p>
                </div>
            </div>
        </div>
        ...
        <div class="p-6 sm:p-8 flex flex-col md:w-1/2 lg:w-3/5 bg-white rounded-b-xl md:rounded-r-xl md:rounded-bl-none">
            <div class="mb-8 fade-in delay-200 flex-grow overflow-y-auto">
                <h2 class="text-3xl font-bold text-gray-800 mb-6 border-b-2 border-purple-200 pb-2 heading-rtl-align" data-key="productPricesHeading"></h2>
    ...
    <script>
        ...
        function updateContent(lang) {
            ...
            productsData.forEach(product => {
                ...
                listItem.className = 'flex justify-between items-center bg-gray-50 p-4 rounded-lg shadow-sm hover:shadow-md transition-shadow duration-200';

                const productNameSpan = document.createElement('span');
                productNameSpan.className = 'text-lg font-medium text-gray-700 product-name';
                productNameSpan.textContent = lang === 'ar' ? product.name_ar : product.name_en;

                const productPriceSpan = document.createElement('span');
                productPriceSpan.className = 'text-xl font-bold text-purple-700 product-price';
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            const qrCodeData = `https://trymajlis.com/checkout?store_id=${storeId}`; 
