...
<head>
    ...
    <style>
        ...
        input:checked + .slider {
            background-color: #EDAA4B;
        }

        input:focus + .slider {
            box-shadow: 0 0 1px #EDAA4B;
        }
...
<?php $currentLang = "ar";?>
<body class="p-4 sm:p-6 md:p-8 bg-DBDAD6 min-h-screen flex flex-col items-center justify-center @if($currentLang === 'ar') rtl @endif">
    ...
    <div class="max-w-4xl w-full bg-white rounded-xl shadow-2xl overflow-hidden flex flex-col transform transition-all duration-300 hover:scale-[1.005] mx-auto">
        ...
        <div class="absolute top-4 right-4 z-10 flex space-x-2 fade-in delay-0" style="display:none">
            <button id="lang-en" class="px-4 py-2 @if($currentLang === 'en') bg-[#EDAA4B] text-white @else bg-gray-200 text-gray-700 @endif rounded-lg shadow-md hover:bg-[#838484] focus:outline-none focus:ring-2 focus:ring-[#EDAA4B] focus:ring-opacity-75 transition duration-200">
                EN
            </button>
            <button id="lang-ar" class="px-4 py-2 @if($currentLang === 'ar') bg-[#EDAA4B] text-white @else bg-gray-200 text-gray-700 @endif rounded-lg shadow-md hover:bg-gray-300 focus:outline-none focus:ring-2 focus:ring-gray-400 focus:ring-opacity-75 transition duration-200">
                AR
            </button>
        </div>

        <div class="p-6 sm:p-8 bg-[#EDAA4B] text-white text-center rounded-t-xl fade-in">
            <h1 class="text-3xl sm:text-4xl font-extrabold mb-2" data-key="checkoutHeader">
                @if($currentLang === 'ar') الدفع @else Checkout @endif
            </h1>
            <p class="text-white text-lg" data-key="reviewOrder">
        ...
        <div class="p-6 sm:p-8 fade-in delay-100">
            <h2 class="text-2xl font-bold text-gray-800 mb-4 border-b-2 border-[#EDAA4B] pb-2" data-key="yourOrder">
            ...
            <ul id="order-summary-list" class="space-y-3 mb-6">
                ...
                @forelse($checkoutItems as $item)
                    <li class="flex justify-between items-center bg-white p-3 rounded-md shadow-sm">
                        ...
                        <span class="font-semibold text-[#838484] w-1/6 @if($currentLang === 'ar') text-left @else text-right @endif">{{ number_format($item['total_item_price'], 2) }}</span>
                    </li>
                @empty
            ...
            <div class="flex justify-between items-center bg-[#DBDAD6] p-4 rounded-lg shadow-inner">
                <span class="text-xl font-bold text-gray-800" data-key="orderTotal">
                    @if($currentLang === 'ar') إجمالي الطلب: @else Order Total: @endif
                </span>
                <span id="final-total-amount" class="text-2xl font-extrabold text-[#838484]">
        ...
        <div class="p-6 sm:p-8 border-t border-gray-200 fade-in delay-200">
            <h2 class="text-2xl font-bold text-gray-800 mb-4 border-b-2 border-[#EDAA4B] pb-2" data-key="customerInformation">
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <input type="text" id="customer-name" placeholder="@if($currentLang === 'ar') اختياري  @else Full Name @endif" class="mt-1 block w-full p-3 border border-gray-300 rounded-md shadow-sm focus:ring-[#EDAA4B] focus:border-[#EDAA4B]">
                </div>
                <div>
                    ...
                    <input type="email" id="customer-email" placeholder="@if($currentLang === 'ar') اختياري @else email@example.com @endif" class="mt-1 block w-full p-3 border border-gray-300 rounded-md shadow-sm focus:ring-[#EDAA4B] focus:border-[#EDAA4B]">
                </div>
                <div>
                    ...
                    <input type="tel" id="customer-phone" placeholder="@if($currentLang === 'ar') اختياري @else +1 234 567 8900 @endif" class="mt-1 block w-full p-3 border border-gray-300 rounded-md shadow-sm focus:ring-[#EDAA4B] focus:border-[#EDAA4B]">
                </div>
            </div>
        </div>

        <div class="p-6 sm:p-8 border-t border-gray-200 fade-in delay-200" style="display:none;">
            <h2 class="text-2xl font-bold text-gray-800 mb-4 border-b-2 border-[#EDAA4B] pb-2" data-key="paymentOptions">
                @if($currentLang === 'ar') خيارات الدفع @else Payment Options @endif
            </h2>

            <div class="space-y-4">
                <div class="flex items-center bg-[#DBDAD6] p-4 rounded-lg shadow-sm">
                    <input type="radio" id="payment-applepay" name="payment-method" value="applepay" class="form-radio text-[#EDAA4B] h-5 w-5" checked>
                ...
                <div class="bg-[#DBDAD6] p-4 rounded-lg shadow-sm" style="display:none">
                    <div class="flex items-center justify-between">
                        <div class="flex items-center">
                            <input type="radio" id="payment-card" name="payment-method" value="card" class="form-radio text-[#EDAA4B] h-5 w-5">
                    ...
                    <div id="card-details" class="card-details-section mt-4 pt-6 pb-6 border-t border-gray-200 hidden">
                        <div class="space-y-4">
                            <div>
                                ...
                                <input type="text" id="card-number" placeholder="XXXX XXXX XXXX XXXX" class="mt-1 block w-full p-3 border border-gray-300 rounded-md shadow-sm focus:ring-[#EDAA4B] focus:border-[#EDAA4B]">
                            </div>
                            <div class="flex space-x-4 flex-row-reverse-on-rtl">
                                <div class="flex-1">
                                    ...
                                    <input type="text" id="expiry-date" placeholder="MM/YY" class="mt-1 block w-full p-3 border border-gray-300 rounded-md shadow-sm focus:ring-[#EDAA4B] focus:border-[#EDAA4B]">
                                </div>
                                <div class="flex-1">
                                    <label for="cvv" class="block text-sm font-medium text-gray-700 mb-1" data-key="cvv">
                                        @if($currentLang === 'ar') CVV @else CVV @endif
                                    </label>
                                    <input type="text" id="cvv" placeholder="123" class="mt-1 block w-full p-3 border border-gray-300 rounded-md shadow-sm focus:ring-[#EDAA4B] focus:border-[#EDAA4B]">
                                </div>
                            </div>
                            <div>
                                ...
                                <input type="text" id="card-name" placeholder="Full Name" class="mt-1 block w-full p-3 border border-gray-300 rounded-md shadow-sm focus:ring-[#EDAA4B] focus:border-[#EDAA4B]">
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <div class="p-6 sm:p-8 bg-[#B0ACA7] rounded-b-xl border-t border-gray-200 text-center fade-in delay-300">
            <button id="pay-now-button" class="w-full px-8 py-4 bg-[#EDAA4B] text-white font-bold rounded-lg shadow-lg hover:bg-[#838484] focus:outline-none focus:ring-2 focus:ring-[#EDAA4B] focus:ring-opacity-75 transition duration-300 ease-in-out transform hover:scale-105">
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            payNowButton.addEventListener('click', async () => {
                ...
                try {
                    ...
                    const termUrl3ds = 'https://waslqr.com/thank-you';
                    const random = confirmOrderData.order_id;// Math.floor(1000 + Math.random() * 9000); // Simple unique order ID for Edfapay

                   const edfapayUrl = `https://waslqr.com/edfapay?order_id=${random}&order_amount=${orderAmount}&order_currency=SAR&payer_first_name=${customerName}&payer_last_name=${customerName}&payer_country=SA&payer_city=Riyad&payer_email=${customerEmail}&payer_phone=${customerPhone}&term_url_3ds=https://waslqr.com/payment-processing/${random}`;
